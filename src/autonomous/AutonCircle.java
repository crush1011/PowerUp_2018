...
public class AutonCircle implements Runnable {
	...
	double angle, topSpeed, distance, radius;
	double rotateOutput;
	...
	public AutonCircle(DriveTrain driveTrain, NavX navx, double radius, double topSpeed, double angle) {

		this.angle = angle;
		this.radius = radius;
		this.topSpeed = topSpeed;
		this.distance = (angle/360) * 2 * Math.PI * radius;  
	...
	public void run() {
		...
		if(right) {
			rightMod = radius / (radius + wheelDistance); 
		}
		if(!right) {
			leftMod = radius / (radius + wheelDistance);
		}
		...
		while (!stop && DriverStation.getInstance().isAutonomous() && DriverStation.getInstance().isEnabled()) {
			...
			if(right) {
				distanceTravelled = Math.abs(Systems.getInstance().getEncoderDistance(SysObj.Sensors.LEFT_ENCODER) - initialPos);
			}
			if(!right) {
				distanceTravelled = Math.abs(Systems.getInstance().getEncoderDistance(SysObj.Sensors.RIGHT_ENCODER) - initialPos);
			}
			...
			drive.tankDrive(leftMod * (actualVelocity + (kA * currentAcceleration)), rightMod * (actualVelocity + (kA * currentAcceleration)));
			if (backwards) {
